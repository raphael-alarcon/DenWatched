...
public class WatchedModGenericBlock extends Block implements IHasModel {

    public WatchedModGenericBlock(String name, Material materialIn) {
        super(materialIn);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            if (playerIn.getHeldItemMainhand().getItem().equals(ModItems.usbkey) || playerIn.getHeldItemOffhand().getItem().equals(ModItems.usbkey)) {
                return true;
            }
        }
        return false;
